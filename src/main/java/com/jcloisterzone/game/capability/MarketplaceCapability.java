...
//    					}
                ...
                ExprItem expr = new ExprItem(tiles, "marketplace", tiles);
                for (Player player: owners) {
                    bonusPoints = bonusPoints.append(new ScoreEvent.ReceivedPoints(new PointsExpression("marketplace", expr), player, new ScorePositionsFeaturePointer(fps.get(), positions))); // Positions or Feature pointer
                }
        	}
        }
        return bonusPoints;
